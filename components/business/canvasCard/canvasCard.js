...
Component({
  ...
  methods: {
    drawing(avatarUrl) {
      ...
      let callback = (url => {
        base64src(url, rtn => this.triggerEvent('getCreatedImg', rtn))
      })
      // const ctx = wx.createCanvasContext('cardCanvas', this)
      switch(this.data.type) {
        case 'recruiter':
          getRecruiterCardApi({id: info.uid, type: 'recruiter'}).then(res => callback(res.data.url))
        ...
        break
        case 'position':
          getPositionCardApi({id: info.id, type: 'position'}).then(res => callback(res.data.url))
          // ctx.setTextAlign('left')
        ...
        break
        case 'resume':
          getResumeCardApi({id: info.uid, type: 'resume'}).then(res => {
            base64src(res.data.url, imgSrc => this.triggerEvent('getCreatedImg', imgSrc))
          })
          // ctx.setTextAlign('left')
        ...
        break
        case 'company':
          getCompanyCardApi({id: info.id, type: 'company'}).then(res => callback(res.data.url))
        ...
        break
        case 'interview':
          getInterviewCardApi({id: info.interviewId, type: 'interview'}).then(res => callback(res.data.url))
          // ctx.setTextAlign('left')
